...
mv erpnext-nginx.conf /etc/nginx/sites-available/erpnext
ln -s /etc/nginx/sites-available/erpnext /etc/nginx/sites-enabled/erpnext
docker compose up -d
#local_ip=$(ip route get 1 | awk '{print $7}')
#echo "Now you can access tunnel through URL: https://$url"
#sleep 5
echo "To Run Behind nginx proxy please set server_name in /etc/nginx/sites-enabled/erpnext"